...
class OrderController
{
    ...
    public function index(): void
    {
        ...
        $statuses = Request::query('statuses'); // e.g. "pending,preparing,served"

        if ($statuses) {
            $allowed  = ['pending','preparing','served','checkout_requested','completed','cancelled'];
            $list     = array_filter(
    ...
    public function store(): void
    {
        ...
        // ─── Status logic per order type ──────
        // Dine-in: goes straight to "preparing" (kitchen starts immediately)
        // Delivery/Pickup: goes to "pending" (cashier accepts first)
        $isDineIn      = ($type === 'dine_in');
        $initialStatus = $isDineIn ? 'preparing' : 'pending';
        $paymentStatus = 'pending'; // always pending until cashier marks paid
        $acceptedAt    = $isDineIn ? now() : null;
        ...
        $pdo->prepare("
            INSERT INTO orders
                (branch_id, user_id, guest_id, order_number, type, status,
                 ...
                 estimated_ready_at, accepted_at, created_at, updated_at)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
        ")->execute([
            $branchId,
    ...
    // Only cashier and chef can update statuses (manager excluded from operations)
    public function updateStatus(array $params): void
    {
        $user = Auth::requireRole(['super_admin', 'owner', 'cashier', 'chef']);
        $data = Request::validate([
            'status' => 'required|in:preparing,served,checkout_requested,completed,cancelled,accepted,ready,delivered',
        ]);
        ...
        // Chef: can only move dine-in orders from preparing → served
        // Cashier: handles everything else
        if ($role === 'chef') {
            if (!$isDineIn) {
                Response::forbidden('Chefs can only update dine-in orders.');
            }
            if ($data['status'] !== 'served') {
                Response::forbidden('Chefs can only mark orders as served.');
            }
            if ($order['status'] !== 'preparing') {
                Response::error("Cannot mark as served from status '{$order['status']}'.", 422);
            }
        }
        ...
        if ($isDineIn) {
            // Dine-in flow:
            // pending → preparing (auto on create, but cashier can also trigger)
            // preparing → served (chef marks food delivered to table)
            // served → checkout_requested (cashier marks customer requested bill)
            // checkout_requested → completed (after payment via /checkout endpoint)
            // any non-completed → cancelled
            $allowed = [
                'pending'            => ['preparing', 'cancelled'],
                'preparing'          => ['served', 'cancelled'],
                'served'             => ['checkout_requested', 'cancelled'],
                'checkout_requested' => ['completed', 'cancelled'],
            ];
        } else {
            // Delivery / Pickup flow:
            // pending → accepted (cashier accepts) → preparing (cashier starts) →
            // ready (cashier marks ready) → delivered/completed (cashier closes)
            $allowed = [
        ...
        if (!in_array($data['status'], $allowed[$order['status']] ?? [])) {
            Response::error(
                "Cannot change status from '{$order['status']}' to '{$data['status']}'.", 422
            );
        }
    ...
    // Cashier records payment and closes the order (dine-in only)
    public function checkout(array $params): void
    {
        ...
        // Must be in checkout_requested or served state to process payment
        if (!in_array($order['status'], ['checkout_requested', 'served'])) {
            Response::error(
                "Order must be in 'served' or 'checkout_requested' status to process payment. Current: '{$order['status']}'.",
                422
            );
        }
    ...
    // PATCH /orders/:id/request-checkout
    // Cashier or customer signals they want the bill (dine-in)
    public function requestCheckout(array $params): void
    {
        $user = Auth::requireRole(['super_admin', 'owner', 'cashier']);
        $pdo  = Database::get();
        ...
        if (!$order) Response::notFound('Order not found.');
        if (!Auth::canAccessBranch($user, (int)$order['branch_id'])) Response::forbidden();
        if ($order['type'] !== 'dine_in') Response::error('Only for dine-in orders.', 422);

        if (!in_array($order['status'], ['served', 'preparing'])) {
            Response::error("Cannot request checkout from status '{$order['status']}'.", 422);
        }

        $pdo->prepare("UPDATE orders SET status = 'checkout_requested', updated_at = ? WHERE id = ?")
            ->execute([now(), $params['id']]);

        $stmt = $pdo->prepare('SELECT * FROM orders WHERE id = ?');
        $stmt->execute([$params['id']]);

        Response::success($this->attachItems($pdo, $stmt->fetch()), 'Checkout requested.');
    }
